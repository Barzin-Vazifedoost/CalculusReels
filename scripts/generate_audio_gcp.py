...
Usage:
  python generate_audio_gcp.py              # generate all missing
  python generate_audio_gcp.py --force      # regenerate everything  
  python generate_audio_gcp.py --id 3       # regenerate a single topic
"""
...
def generate_audio_for_topic(topic_id, title, content):
    """Generate audio using Google Cloud Text-to-Speech"""
    try:
        # Try to initialize the TTS client - it will use default credentials
        client = texttospeech.TextToSpeechClient()
        
        # Set up the synthesis input with a shorter sample for testing
        sample_text = f"Welcome to {title}. " + content.strip()[:200] + "..."
        synthesis_input = texttospeech.SynthesisInput(text=sample_text)
        
        # Configure voice settings - using a standard voice first
        ...
        print("💡 Hint: You may need to set up Google Cloud authentication.")
...
def main():
    parser = argparse.ArgumentParser(description="Generate calculus audio content using Google Cloud TTS")
    ...
    print(f"🎯 Generating audio for {len(topics_to_process)} topic(s)...")
    ...
    for topic_id, title, content in topics_to_process:
        ...
        if generate_audio_for_topic(topic_id, title, content):
            success_count += 1
    
    print(f"\n🎉 Successfully generated {success_count} audio files!")
    print("🚀 Your CalculusReels app is now ready with AI-generated audio content!")
